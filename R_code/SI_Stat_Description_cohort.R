...
category_summary_gender_cohort <- df_cat %>%
  select(gender,cohort_1, all_of(cate_vars_to_summarize)) %>%
  pivot_longer(
    cols = -c(gender,cohort_1), names_to = "variable", values_to = "category"
  ) %>%
  group_by(cohort_1,variable, gender, category) %>%
...
if (exists("variable_map_1")) {
  category_summary_gender_cohort <- category_summary_gender_cohort %>%
...
# ---- 4) Plot ----
p_cate_cohort <- function(df,var, value, order) {
  
  df_sub <- df[df[[var]] == value, ]
  df_sub$category <- factor(df_sub$category, 
                            levels = order)
...
p_cate_cohort_list <- list()

for (i in unique(category_summary_gender_cohort$cohort_1)) {
  p_cate_cohort_list[[as.character(i)]] <- p_cate_cohort(category_summary_gender_cohort, "cohort_1", i,c("North America", "Europe", "other"))
}
...
survival_data_age_conti_long <- df_cont %>%
  select(gender, scopus_author_id, academic_age, cohort_1, all_of(conti_vars_present)) %>%
...
mean_ci_values <- survival_data_age_conti_long %>%
  group_by(cohort_1, variable,label, gender, academic_age) %>%
  summarise(
...
mean_ci_values <- na.omit(mean_ci_values)
# ---- 5) Plot: all selected continuous variables ----
p_conti_cohort <- function(df,var, value) {
  
  df_subset<-df[df[[var]]==value,]
...
p_conti_cohort_list <- list()

for (i in unique(mean_ci_values$cohort_1)) {
  p_conti_cohort_list[[as.character(i)]] <- p_conti_cohort(mean_ci_values, "cohort_1", i)
}
...
for (i in c("2000","2005","2010")){
  
  panel_plot_list[[i]] <- function(lbl) {
    ggplot(filter(mean_ci_values%>%filter(cohort_1==i), label == lbl),
           aes(x = academic_age, y = mean_value,
               group = gender, color = gender, fill = gender)) +
      geom_line(linewidth = 1.2) +
      geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.3, color = NA) +
...
for (i in c("2000","2005","2010")){
  ...
  panel_plot_sub<-panel_plot_list[[i]]
  
  p1<-panel_plot_sub(lbl[1])+
    geom_text(
      data = label_df,
  ...
  plots_list <- c(
    list(p1),
    lapply(lbl[2:7], panel_plot_sub), # slots 1..7 from your data
    list(p_cate_cohort_list[[i]]),                            # slot 8 = your custom plot
    list(panel_plot_sub(lbl[9]))       # slot 9 from your data
  )
  
  final_3x3_sub <- wrap_plots(plots_list, ncol = 3)
  
  
  ggsave(here::here("plots",paste0("SI_fig_variable_", i, ".pdf")), final_3x3_sub,
